...
\begin{abstract}
    \noindent
    Cambodia's elderly population will reach approximately 2.1 million by 2030, comprising 11\% of total population, amid healthcare challenges including physician shortages and geographic disparities. Cloud-based elderly monitoring systems transmit sensitive data to remote servers, creating surveillance risks with limited data protection enforcement. This study demonstrates how privacy principles can inform architectural decisions from inception rather than being retrofitted post-deployment by validating technical feasibility and cost-effectiveness of edge-based pose estimation for elderly safety monitoring in resource-constrained contexts. The proposed architecture comprises four 850nm NIR cameras and an NVIDIA Jetson Orin Nano edge platform. Privacy protection emerges from system constraints: immediate conversion of video frames to skeletal coordinates, pose-only storage, and permanent deletion of original footage. The pipeline integrates YOLOv8n person detection and MediaPipe pose estimation, achieving 100\% on-device processing. Validation on 20 commercial CCTV videos demonstrates 91.3\% keypoint detection with 20.53 FPS processing speed, confirming 24/7 monitoring capability without facial recognition technology. Cost analysis shows edge architecture reduces 3-year total cost by 61\% compared to cloud alternatives, expanding market accessibility to 168,000-252,000 elderly individuals in middle-income Cambodian urban households, representing 8-12\% of elderly population by 2030. The study demonstrates privacy-first design yields economic co-benefits, though fall detection accuracy requires validation on benchmark datasets with ground truth annotations. This work contributes empirical evidence that governance principles can drive technical architecture in healthcare AI for Cambodia.
...
Since the 2000s, global demographic transitions have constituted a pressing challenge for healthcare systems. Southeast Asia's elderly population is projected to increase from 12.2\% in 2024 to 22.9\% by 2050 \parencite{whoAgeingHealthSEARO2025}, representing a fundamental restructuring of regional age distributions. Cambodia's elderly population (60+ years) will reach approximately 2.1 million by 2030, comprising 11\% of the projected 19 million total population \parencite{unitednationsdepartmentofeconomicandsocialaffairspopulationdivisionDataPortalPopulation2015}, with 8-12\% residing in middle-income urban households (4th-5th income quintiles).
...
Two architectural mechanisms enforce privacy by design \parencite{cavoukianPrivacyDesignEssential2010}:
...
\textbf{Pose-only storage with immediate frame disposal.} The system stores only 17 body keypoints in COCO format (Common Objects in Context standard: shoulders, elbows, wrists, hips, knees, ankles, and torso keypoints), excluding face landmarks entirely. MediaPipe's full pose model detects 33 landmarks, but facial keypoints are intentionally discarded during extraction. Raw video frames are never persisted to storage—not encrypted, not anonymized, simply never written to disk. Video frames undergo processing in real-time, with deletion occurring immediately after keypoint extraction. The temporal gap between frame capture and deletion measures in milliseconds, limited by processing pipeline speed. This design choice eliminates facial recognition capability structurally: no facial data exists to enable identification, and no video retention eliminates retrospective re-identification risk, data breach exposure of video archives, and dependency on deletion policy compliance.
...
\begin{figure}[htbp]
    ...
    \begin{flushleft}
        \footnotesize
        \textit{Note.} Architecture enforces privacy through two mechanisms: (1) 100\% on-device edge processing with zero cloud transmission, and (2) pose-only storage with immediate frame disposal (17 body keypoints, no facial landmarks, video frames deleted immediately after extraction). All computation occurs on local edge device, eliminating network exposure and cloud data sovereignty concerns.
    \end{flushleft}
\end{figure}
...
Cambodia's elderly population (60+ years) will reach approximately 2.1 million by 2030 \parencite{unitednationsdepartmentofeconomicandsocialaffairspopulationdivisionDataPortalPopulation2015}. Estimating that 8-12\% fall within middle-income urban households with financial capacity and motivation to adopt fall detection technology yields 168,000-252,000 potential users.
...
\begin{itemize}
    ...
    \item Confidence: 0.868 vs. 0.833 = 0.035 point improvement
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lccc}
        ...
        Average confidence                & 0.833                     & 0.868                     & +0.035          \\
...
\begin{itemize}
    ...
    \item Middle-income urban elderly (~40\% of urban households): \textbf{168,000 potential adopters}
    \item Conservative estimate (60\% adoption factor): \textbf{252,000 potential adopters}
\end{itemize}

The 168,000-252,000 range represents 8-12\% of Cambodia's projected elderly population. By comparison, the cloud system's higher cost might contract market reach to approximately 5th quintile only: ~126,000 potential adopters (6\% of elderly population), representing 25-50\% smaller market than edge system reach.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lrr}
        ...
        \quad\quad 4th--5th quintile (middle-income)$^b$          & \textbf{168,000}                       & \textbf{8\%}           \\
    ...
    \begin{flushleft}
        ...
        \textit{Note.} Edge system expands reach by 33-100\% vs. cloud alternatives (168,000--252,000 vs. ~126,000 limited to 5th quintile).
    \end{flushleft}
\end{table}
...
The zero-subscription model removes recurring payment barriers particularly challenging for middle-income Cambodian households earning \$870-\$1,622/month, where \$45/month subscription represents 2.8-5.2\% of monthly income indefinitely. The estimated market reach of 168,000-252,000 elderly individuals (8-12\% of Cambodia's projected 2030 elderly population) represents meaningful expansion beyond high-income early adopters who can afford subscription-based cloud monitoring.
...
\textbf{Market accessibility:} The 168,000-252,000 user estimate represents economic feasibility (households with financial capacity) but not actual adoption. Perceived need, technology trust, family dynamics, and alternative options beyond cost determine technology adoption.
